...
namespace ProjectEsky.Networking
{
    public class EskyNetworkEntity : MonoBehaviour
    {
        ...
        public virtual void UpdateTrackingLocally()
        {
            Debug.Log("Updating Tracking Locally via the Esky Network Entity");
        ...
        public void Update()
        {
            ...
            { // this entities information is controlled by another client
                if (closestAnchor != null)
                {
                    Vector3 targetPosition = closestAnchor.transform.TransformPoint(LocalPosition);
                    Quaternion targetRotation = Quaternion.Euler(closestAnchor.transform.TransformDirection(LocalRotation));
