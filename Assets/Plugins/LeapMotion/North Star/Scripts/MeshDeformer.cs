...
namespace Leap.Unity.AR {
  public class MeshDeformer {
    ...
    public MeshDeformer(MeshFilter inputMeshFilter, int[] controlVertices, float weight = 1f) {
      ...
      originalControlPoints = new Vector3[controlVertices.Length];
      currentControlPoints = new Vector3[controlVertices.Length];
      originalMeshVertices = inputMeshFilter.sharedMesh.vertices;
      for (int i = 0; i < controlVertices.Length; i++) {
        originalControlPoints[i] = originalMeshVertices[controlVertices[i]];
      ...
      initializeMeshDeformation(inputMeshFilter.sharedMesh, originalControlPoints, ref originalMeshVertices,
    ...
    public void updateMeshDeformation(ref MeshFilter filter, Vector3[] controlPoints, bool useRotation = true, float weight = 1f, int iters = 10) {
      for (int i = 0; i < controlPoints.Length; i++) {
        currentControlPoints[i] = controlPoints[i];
      }
      Mesh toDistort = filter.sharedMesh;
